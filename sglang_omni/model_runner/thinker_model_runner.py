...
class ThinkerModelRunner(ModelRunner):
    """Thinker: injects multimodal embeddings in prepare_forward()."""
    ...
    def _get_inner_model_components(model):
        ...
        if embed_tokens is None:
            get_fn = getattr(inner, "get_input_embeddings", None)
            if callable(get_fn):
                embed_tokens = get_fn()
        if embed_tokens is None:
    ...
    def _forward_with_omni_embeds(
        self, forward_batch, input_embeds, deepstack_visual_embeds=None, visual_pos_masks=None,
    ):
        ...
        if deepstack_visual_embeds is not None and visual_pos_masks is not None:
            ...
            full_ds = torch.zeros(input_embeds.shape[0], ds_input.shape[-1], device=device, dtype=dtype)
        ...
        logits_output = outer.logits_processor(
            forward_batch.input_ids, hidden_states, outer.lm_head, forward_batch,
        )
