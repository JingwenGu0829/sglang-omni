...
def load_code2wav_model(model_path: str, *, device: str = "cuda", dtype: str | None = None):
    ...
    model = load_module(model, model_path, prefix="code2wav.", dtype=torch_dtype, device=device, strict=False)
    return model
...
class Code2WavScheduler:
    ...
    def start(self) -> None:
        ...
        while self._running:
            ...
            if msg.type == "new_request":
                self._code_chunks[msg.request_id] = []
                self._emitted[msg.request_id] = 0
                self._audio_chunks[msg.request_id] = []
    ...
    def _on_chunk(self, request_id: str, chunk: Any) -> None:
        ...
        codes = chunk.data.to(device=self._device, dtype=torch.long) if hasattr(chunk, "data") else chunk.to(device=self._device, dtype=torch.long)
    ...
    def _decode_and_emit(self, request_id: str) -> None:
        ...
        audio = self._decode_incremental(chunks, start, end)
        self._emitted[request_id] = end
        if audio.size > 0:
            self._audio_chunks[request_id].append(audio)

    def _decode_incremental(self, code_chunks, start, end) -> np.ndarray:
        if start >= end:
